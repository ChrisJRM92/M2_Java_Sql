package com.clearminds.maquina;

import com.clearminds.componentes.Celda;
import com.clearminds.componentes.Producto;

public class MaquinaDulces {
    private Celda celda1;
    private Celda celda2;
    private Celda celda3;
    private Celda celda4;
    private double saldo;

    public void configurarMaquina(String code1, String code2, String code3, String code4){
        celda1 = new Celda(code1);
        celda2 = new Celda(code2);
        celda3 = new Celda(code3);
        celda4 = new Celda(code4);
    }

    public void mostrarConfiguracion(){
        System.out.println("Celda 1: "+celda1.getCodigo());
        System.out.println("Celda 2: "+celda2.getCodigo());
        System.out.println("Celda 3: "+celda3.getCodigo());
        System.out.println("Celda 4: "+celda4.getCodigo());
    }

    public Celda buscarCelda(String codigo){
        if(this.celda1.getCodigo()==codigo){
            return this.celda1;
        }else if (this.celda2.getCodigo()==codigo) {
            return this.celda2;
        }else if (this.celda3.getCodigo()==codigo){
            return this.celda3;
        }else if (this.celda4.getCodigo()==codigo){
            return this.celda4;
        }else {
            return null;
        }
    }

    public void cargarProducto(Producto producto, String codigo, int cantidad){
        Celda celdaRecuperada = buscarCelda(codigo);
        celdaRecuperada.ingresarProducto(producto, cantidad);

    }

    public void mostrarProductos(){
        System.out.println("***********CELDA " + this.celda1.getCodigo());
        System.out.println("Stock: "+this.celda1.getStock());
        if (celda1.getProducto()==null){
            System.out.println("La celda no tiene producto!!");
        }else{
            System.out.println("Nombre: "+this.celda1.getProducto().getNombre());
            System.out.println("Precio: "+this.celda1.getProducto().getPrecio());
            System.out.println("Codigo: "+this.celda1.getProducto().getCodigo());
        }

        System.out.println("***********CELDA " + this.celda2.getCodigo());
        System.out.println("Stock: "+this.celda2.getStock());
        if (celda2.getProducto()==null){
            System.out.println("La celda no tiene producto!!");
        }else{
            System.out.println("Nombre: "+this.celda2.getProducto().getNombre());
            System.out.println("Precio: "+this.celda2.getProducto().getPrecio());
            System.out.println("Codigo: "+this.celda2.getProducto().getCodigo());
        }

        System.out.println("***********CELDA " + this.celda3.getCodigo());
        System.out.println("Stock: "+this.celda3.getStock());
        if (celda3.getProducto()==null){
            System.out.println("La celda no tiene producto!!");
        }else{
            System.out.println("Nombre: "+this.celda3.getProducto().getNombre());
            System.out.println("Precio: "+this.celda3.getProducto().getPrecio());
            System.out.println("Codigo: "+this.celda3.getProducto().getCodigo());
        }

        System.out.println("***********CELDA " + this.celda4.getCodigo());
        System.out.println("Stock: "+this.celda4.getStock());
        if (celda4.getProducto()==null){
            System.out.println("La celda no tiene producto!!");
        }else{
            System.out.println("Nombre: "+this.celda4.getProducto().getNombre());
            System.out.println("Precio: "+this.celda4.getProducto().getPrecio());
            System.out.println("Codigo: "+this.celda4.getProducto().getCodigo());
        }
        System.out.println("Saldo: "+this.saldo);
    }

    public Producto buscarProductoEnCelda(String codigo){
        if(this.celda1.getCodigo() == codigo){
            return celda1.getProducto();
        } else if (this.celda2.getCodigo() == codigo) {
            return celda2.getProducto();
        } else if (this.celda3.getCodigo() == codigo) {
            return celda3.getProducto();
        } else if (this.celda4.getCodigo() == codigo) {
            return celda4.getProducto();
        }else {
            return null;
        }
    }

    public double consultarPrecio(String codigo){
        if(this.celda1.getCodigo() == codigo){
            return celda1.getProducto().getPrecio();
        } else if (this.celda2.getCodigo() == codigo) {
            return celda2.getProducto().getPrecio();
        } else if (this.celda3.getCodigo() == codigo) {
            return celda3.getProducto().getPrecio();
        } else if (this.celda4.getCodigo() == codigo) {
            return celda4.getProducto().getPrecio();
        }else {
            return 0;
        }
    }

    public Celda buscarCeldaProducto(String codigo){
        if(this.celda1.getProducto() != null && this.celda1.getProducto().getCodigo() == codigo){
            return this.celda1;
        } else if (this.celda2.getProducto() != null && this.celda2.getProducto().getCodigo()==codigo) {
            return  this.celda2;
        } else if (this.celda3.getProducto() != null && this.celda3.getProducto().getCodigo()==codigo) {
            return this.celda3;
        } else if (this.celda4.getProducto() !=null && this.celda4.getProducto().getCodigo() == codigo) {
            return this.celda4;
        }else {
            return null;
        }
    }

    public void incrementarProductos(String codigo, int cantidad){
        Celda celdaEncontrada = buscarCeldaProducto(codigo);
        celdaEncontrada.setStock(celdaEncontrada.getStock()+cantidad);
    }

    public void vender(String codigo){
        Producto productoEncontrado = buscarProductoEnCelda(codigo);
        if(this.celda1.getProducto() != null && this.celda1.getProducto().getNombre() == productoEncontrado.getNombre()){
            this.celda1.setStock(this.celda1.getStock()-1);
            this.saldo +=productoEncontrado.getPrecio();
        }

        if(this.celda2.getProducto() != null && this.celda2.getProducto().getNombre() == productoEncontrado.getNombre()){
            this.celda2.setStock(this.celda2.getStock()-1);
            this.saldo +=productoEncontrado.getPrecio();
        }

        if(this.celda3.getProducto() != null && this.celda3.getProducto().getNombre() == productoEncontrado.getNombre()){
            this.celda3.setStock(this.celda3.getStock()-1);
            this.saldo +=productoEncontrado.getPrecio();
        }

        if(this.celda4.getProducto() != null && this.celda4.getProducto().getNombre() == productoEncontrado.getNombre()){
            this.celda4.setStock(this.celda4.getStock()-1);
            this.saldo +=productoEncontrado.getPrecio();
        }

        //mostrarProductos();
    }

    public double venderConCambio (String codigo, int valor){
        if(this.celda1.getProducto()!=null && this.celda1.getCodigo() == codigo){
            this.celda1.setStock(this.celda1.getStock()-1);
            saldo+=this.celda1.getProducto().getPrecio();
            System.out.println("Saldo: "+this.saldo);
            double vuelto = valor-this.celda1.getProducto().getPrecio();
            return vuelto;
        } else if (this.celda2.getProducto()!=null && this.celda2.getCodigo() == codigo) {
            this.celda2.setStock(this.celda2.getStock()-1);
            saldo+=this.celda2.getProducto().getPrecio();
            System.out.println("Saldo: "+this.saldo);
            double vuelto = valor-this.celda2.getProducto().getPrecio();
            return vuelto;
        } else if (this.celda3.getProducto()!=null && this.celda3.getCodigo() == codigo) {
            this.celda3.setStock(this.celda3.getStock()-1);
            saldo+=this.celda3.getProducto().getPrecio();
            System.out.println("Saldo: "+this.saldo);
            double vuelto = valor-this.celda3.getProducto().getPrecio();
            return vuelto;
        } else if (this.celda4.getProducto()!=null && this.celda4.getCodigo() == codigo) {
            this.celda4.setStock(this.celda4.getStock()-1);
            saldo+=this.celda4.getProducto().getPrecio();
            System.out.println("Saldo: "+this.saldo);
            double vuelto = valor-this.celda4.getProducto().getPrecio();
            return vuelto;
        }else {
            return this.saldo;
        }
    }

    public Celda getCelda1() {
        return celda1;
    }

    public void setCelda1(Celda celda1) {
        this.celda1 = celda1;
    }

    public Celda getCelda2() {
        return celda2;
    }

    public void setCelda2(Celda celda2) {
        this.celda2 = celda2;
    }

    public Celda getCelda3() {
        return celda3;
    }

    public void setCelda3(Celda celda3) {
        this.celda3 = celda3;
    }

    public Celda getCelda4() {
        return celda4;
    }

    public void setCelda4(Celda celda4) {
        this.celda4 = celda4;
    }

    public double getSaldo() {
        return saldo;
    }

    public void setSaldo(double saldo) {
        this.saldo = saldo;
    }
}
